...
def buildTFNeuralNet(x, y, eps = 1):
    model = tf.keras.models.Sequential([tf.keras.layers.Flatten(), tf.keras.layers.Dense(512, activation=tf.nn.relu), tf.keras.layers.Dense(NUM_CLASSES, activation=tf.nn.softmax)])
...
def buildTFConvNet(x, y, eps = 10, dropout = True, dropRate = 0.18):
    ...
    model.add(Conv2D(64*2, kernel_size=3, activation='relu'))
    model.add(Conv2D(64*2, kernel_size=3, activation='relu'))
    model.add(BatchNormalization())
    model.add(MaxPooling2D(pool_size=(2, 2)))
    ...
    model.add(Conv2D(64*2*2, kernel_size=3, activation='relu'))
